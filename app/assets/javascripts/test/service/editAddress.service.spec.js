(function () {
    ...
    describe('editAddressService', function () {
        ...
        beforeEach(inject(function (_editAddressService_, _$q_, _$rootScope_, geoDataService, ___) {
            ...
            spyOn(geoDataService, 'getCountries').and.returnValue($q.resolve(countries));
        }));

        describe('getAddressTypes', function () {
            it('should return an array', function () {
                expect(editAddressService.getAddressTypes()).toEqual(jasmine.any(Array));
            });
        });
        ...
        describe('isAddressValid', function () {
            it('should treat addresses without a type as invalid', function () {
                expect(editAddressService.isAddressValid({ address_type: null })).toBe(false);
            });

            it('should treat addresses with a type as valid', function () {
                expect(editAddressService.isAddressValid({ address_type: 'type' })).toBe(true);
            });
        });
        ...
        describe('getAddressTemplate', function () {
            ...
            it('should choose the first available address type', function () {
                ...
                expect(editAddressService.getAddressTemplate({
                    id: 1,
                    addresses: []
        ...
        describe('getCountryOptions', function () {
            it('should prioritize certain countries', asynchronous(function () {
                return editAddressService.getCountryOptions().then(function (countryOptions) {
                    expect(_.map(countryOptions, 'shortCode')).toEqual(['US', 'CA', 'AA', 'BA']);
                });
            }));
        });

        describe('getRegionOptions', function () {
            it('should return the country\'s regions', asynchronous(function () {
                return editAddressService.getRegionOptions('US').then(function (regionOptions) {
                    expect(_.map(regionOptions, 'shortCode')).toEqual(['AL', 'AK', 'AZ']);
                });
            }));

            it('should return an empty array for an invalid country', asynchronous(function () {
                return editAddressService.getRegionOptions('foo').then(function (regionOptions) {
