(function () {
    ...
    describe('errorService', function () {
        ...
        describe('autoRetry', function () {
            it('does nothing when the promise resolves', asynchronous(function () {
                var result = 123;
                var getPromise = jasmine.createSpy('getPromise').and.returnValue($q.resolve(result));
            ...
            it('retries and eventually resolves when the promise rejects once', asynchronous(function () {
                var result = 123;
                var getPromise = jasmine.createSpy('getPromise').and.returnValues(
                    $q.reject(),
                    $q.resolve(result)
                );
                var decorated = errorService.autoRetry(getPromise, { retryCount: 3 });
            ...
            it('retries and eventually rejects when the promise always rejects', asynchronous(function () {
                var error = new Error();
            ...
            it('does not retry errors that do not match the filter', asynchronous(function () {
                var error = new Error();
                var result = 123;
                var getPromise = jasmine.createSpy('getPromise').and.returnValues(
                    $q.reject(error),
