...
// Constants
let userPreferencesService, _;
const httpProxy = jasmine.createSpyObj('httpProxy', ['put']);
...
describe('userPreferences service', function () {
  ...
  describe('organizationOrderChange', function () {
    it('should save the new organization order', function () {
      ...
      expect(userPreferencesService._updateUserPreferences).toHaveBeenCalledWith(
        {
  ...
  describe('applyUserOrgDisplayPreferences', function () {
    it('should sort by ancestry and name with no org order', function () {
      this.orgOrder = null;
      expect(_.map(userPreferencesService.applyUserOrgDisplayPreferences(this.orgs), 'id')).toEqual([
        '3',
        '2',
        '5',
        '1',
    ...
    it('should sort by org order with complete org order', function () {
      ...
      expect(_.map(userPreferencesService.applyUserOrgDisplayPreferences(this.orgs), 'id')).toEqual(this.orgOrder);
    });
