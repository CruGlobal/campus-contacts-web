...
// Constants
let httpProxy, $rootScope, $http, $q, JsonApiDataStore, RequestDeduper;
...
describe('HttpProxyService Tests', function () {
  beforeEach(function () {
    ...
    angular.mock.module(function ($provide) {
      $provide.factory('$http', function ($q) {
        const $http = jasmine.createSpy('$http').and.returnValue($q.resolve(_this.httpResponse));
  ...
  describe('callHttp', function () {
    ...
    it('should get the base API url from envService', inject(function (envService) {
      ...
      const httpConfig = $http.calls.argsFor(0)[0];
    ...
    it(
      ...
      asynchronous(function () {
        const originalAuthorization = 'Bearer old';
        this.token = null;
  ...
  describe('model loading', function () {
    ...
    describe('getUnloadedRelationships', function () {
      it('should treat all relationships as unloaded when the model is null', function () {
        ...
        expect(httpProxy.getUnloadedRelationships(null, this.relationships)).toEqual(this.relationships);
      });

      it('should treat all relationships as unloaded when the model is a placeholder', function () {
        this.relationships = ['a', 'b', 'c'];
        expect(httpProxy.getUnloadedRelationships(this.placeholderModel, this.relationships)).toEqual(
          this.relationships,
        );
      });
      ...
      it('should correctly detect unloaded relationships', function () {
        ...
        this.relationships = ['a', 'b', 'c', 'd', 'e', 'f.a', 'g.a', 'h.a', 'i.a.b.c', 'j.a.b.c', 'k.a.b.c'];
        expect(httpProxy.getUnloadedRelationships(this.model, this.relationships)).toEqual([
          'a',
          'd',
          'e',
          'g.a',
    ...
    describe('getModel', function () {
      ...
      it(
        ...
        asynchronous(function () {
          ...
          const _this = this;
          return httpProxy
            .getModel(this.url, this.type, this.id, this.relationships, this.config)
            .then(function (model) {
              expect(model).toBe(_this.model);
