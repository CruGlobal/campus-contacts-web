...
function reportsService($q, httpProxy, modelsService, JsonApiDataStore, periodService, _) {
  ...
  function createReport(type, reportId) {
    const record = JsonApiDataStore.store.sync({
      data: {
        type,
        id: reportId,
        attributes: {
  ...
  var reportsService = {
    ...
    loadPersonReport: function (organizationId, personId) {
      ...
      const reportId = calculatePersonReportId(organizationId, personId);
      const report = findReport('person_report', reportId);
    ...
    loadOrganizationReports: function (orgs) {
      const organizationIds = _.map(orgs, 'id');
      ...
      const unloadedOrgIds = organizationIds.filter(function (organizationId) {
        const report = reportsService.lookupOrganizationReport(organizationId);
      ...
      return httpProxy
        .get(
          modelsService.getModelMetadata('organization_report').url.all,
          {
            period: periodService.getPeriod(),
    ...
    getInteractionCount: function (report, interactionTypeId) {
      const interaction =
        report &&
        _.find(report.interactions, ({ interaction_type_id }) => `${interaction_type_id}` === interactionTypeId);
    ...
    incrementReportInteraction: function (report, interactionTypeId) {
      const interaction = _.find(report.interactions, {
