...
function loggedInPerson(httpProxy, modelsService, organizationService, permissionService, _) {
  ...
  return {
    ...
    isAdminAt: function (org) {
      const adminOrgIds = _.chain(person.organizational_permissions)
        .filter((orgPermission) => `${orgPermission.permission_id}` === permissionService.adminId)
        .map('organization_id')
        .value();
      const orgAndAncestry = organizationService.getOrgHierarchyIds(org);
    ...
    isDirectAdminAt: (org) => {
      return person.organizational_permissions.some(
        ({ permission_id, organization_id }) =>
          `${permission_id}` === permissionService.adminId && organization_id === org.id,
      );
    },
