...
function groupsService($q, httpProxy, JsonApiDataStore, modelsService, moment, _) {
  var groupsService = {
    // Return a group with default field values
    getGroupTemplate: function (orgId) {
      const group = new JsonApiDataStore.Model('group');
    ...
    createGroup: function (group, leaders) {
      ...
      const payload = payloadFromGroup(group, leaders);
      return httpProxy
        .post(modelsService.getModelMetadata('group').url.all, payload, {
    ...
    updateGroup: function (group, leaders) {
      ...
      const payload = payloadFromGroup(group, leaders);
      return httpProxy
        .put(modelsService.getModelMetadata('group').url.single(group.id), payload, {
          params: { include: 'group_memberships' },
    ...
    deleteGroup: function (group) {
      return httpProxy
        .delete(
          modelsService.getModelMetadata('group').url.single(group.id),
          {},
          {
    ...
    getAllMembers: function (group) {
      return _.reduce(
        ...
        function (allMembers, groupMembership) {
          ...
          if (!groupMembership._placeHolder && groupMembership.person) {
    ...
    getMembersWithRole: function (group, role) {
      return _.chain(group.group_memberships).filter({ role }).map('person').value();
    },

    // Find the group membership for a particular person
    findMember: function (group, person) {
      return _.find(group.group_memberships, { person }) || null;
    },
  ...
  function modelFromRoleChange(group, person, newRole) {
    const groupMembership = _.find(group.group_memberships, {
      person,
    });
  ...
  function payloadFromGroup(group, leaders) {
    const previousLeaders = groupsService.getMembersWithRole(group, 'leader');
    const previousMembers = groupsService.getMembersWithRole(group, 'member');
    const previousAnyRole = groupsService.getAllMembers(group);

    // Determine which leaders were removed
    const demotedLeaders = _.difference(previousLeaders, leaders);

    // Change the demoted members' role to "member"
    const demoteModels = demotedLeaders.map(function (person) {
      return modelFromRoleChange(group, person, 'member');
    });

    // Determine which added leaders were previously members
    const promotedMembers = _.intersection(leaders, previousMembers);

    // Change the promoted members' role to "leader"
    const promoteModels = promotedMembers.map(function (person) {
      return modelFromRoleChange(group, person, 'leader');
    });

    // Determine which of the added leaders were not previously members
    const createdLeaders = _.difference(leaders, previousAnyRole);

    // Create the new members
    const createModels = createdLeaders.map(function (person) {
      const groupMembership = new JsonApiDataStore.Model('group_membership');
    ...
    // Apply the changes
    const includedAttrs = ['name', 'location', 'meets', 'meeting_day', 'start_time', 'end_time'];
    let relationships = [];
  ...
  function loadMemberships(organization) {
    ...
    const allModelsLoaded = organization.groups.every(function (group) {
      return httpProxy.isLoaded(group) && group.group_memberships.every(httpProxy.isLoaded);
    });
    if (allModelsLoaded) {
      return $q.resolve();
    }
  ...
  function loadMissingLeaders(organization) {
    ...
    const missingLeaderIds = _.chain(organization.groups)
