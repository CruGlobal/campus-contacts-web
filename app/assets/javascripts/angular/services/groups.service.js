...
function groupsService ($q, httpProxy, JsonApiDataStore, modelsService, moment, _) {
    ...
    function modelFromRoleChange (group, person, newRole) {
        var groupMembership = _.find(group.group_memberships, { person: person });
        groupMembership.role = newRole;
        return groupMembership.serialize({ attributes: ['role'], relationships: [] }).data;
    }
    ...
    function payloadFromGroup (group, leaders) {
        ...
        var createModels = createdLeaders.map(function (person) {
            var groupMembership = new JsonApiDataStore.Model('group_membership');
    ...
    function loadMemberships (organization) {
        ...
        return httpProxy.get(modelsService.getModelMetadata('group').url.all, {
