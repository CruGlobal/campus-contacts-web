angular.module('campusContactsApp').factory('organizationService', organizationService);
...
function organizationService($q, httpProxy, modelsService, JsonApiDataStore, _) {
  ...
  const organizationService = {
    ...
    getOrgHierarchy: function (org) {
      const hierarchyIds = _.difference(organizationService.getOrgHierarchyIds(org), [...inaccessibleOrgs]);
      const unloadedOrgIds = hierarchyIds.filter(function (orgId) {
        return !httpProxy.isLoaded(organizationService.lookupOrg(orgId));
      });

      return organizationService
        .loadOrgsById(unloadedOrgIds, 'error.messages.organization.load_ancestry')
        .then(function (loadedOrgs) {
          // Mark as inaccessible orgs that we attempted to load but were not included in the response
          const newInaccessibleOrgs = _.difference(unloadedOrgIds, _.map(loadedOrgs, 'id'));
          inaccessibleOrgs = new Set([...inaccessibleOrgs, ...newInaccessibleOrgs]);
    ...
    searchOrgs: function (org, query, deduper) {
      const rootOrgId = organizationService.getOrgHierarchyIds(org)[0];
      return httpProxy
        .get(
          '/organization_search',
          {
    ...
    saveOrg: (org) => {
      return httpProxy
        .put(
          modelsService.getModelMetadata('organization').url.single(org.id),
          {
            data: {
              type: 'organization',
              attributes: {
    ...
    deleteOrg: (org) => {
      return httpProxy.delete(modelsService.getModelMetadata('organization').url.single(org.id), null, {
