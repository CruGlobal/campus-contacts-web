angular.module('campusContactsApp').factory('analyticsService', analyticsService);

function analyticsService($window, envService, $location, loggedInPerson, authenticationService) {
  const initGoogle = () => {
  ...
  const setupAuthenitcatedAnalyticData = () => {
    return loggedInPerson.loadOnce().then(({ thekey_uid, fb_uid, global_registry_mdm_id }) => {
  ...
  const loadAdobeScript = () => {
    const url =
      envService.is('production') || envService.is('productionCampusContacts')
  ...
  const trackAdobe = () =>
    ...
    $window.document.querySelector('body').dispatchEvent(new CustomEvent('content: all pages'));
  ...
  return {
    ...
    track: (transition) => {
      ...
      const currentUrl = newState.path.reduce((acc, p) => {
        ...
        const url = Object.entries(transition.params()).reduce((acc, [paramId, paramValue]) => {
      ...
      if (authenticationService.isTokenValid() && !$window.digitalData.user) {
        setupAuthenitcatedAnalyticData().then(() => {
          trackAdobe();
          angular.isFunction($window.ga) && $window.ga('send', 'pageview', fields);
        });
      } else {
        trackAdobe();
        angular.isFunction($window.ga) && $window.ga('send', 'pageview', fields);
