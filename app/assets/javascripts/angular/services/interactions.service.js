angular.module('campusContactsApp').factory('interactionsService', interactionsService);

// This service contains action logic that is shared across components
function interactionsService(httpProxy, modelsService, JsonApiDataStore, reportsService, loggedInPerson, _) {
  // Fields:
  ...
  return {
    ...
    recordInteraction: function (interaction, organizationId, personId) {
      ...
      const newInteraction = JsonApiDataStore.store.sync({
        data: {
          type: 'interaction',
          attributes: {
      ...
      const createJson = newInteraction.serialize();
      createJson.included = [
        {
      ...
      return httpProxy
        .post(modelsService.getModelMetadata('interaction').url.root, createJson, {
        ...
        .then(function (interaction) {
          ...
          const filteredInteraction = interaction.initiators.filter(
            (initiator, index) => interaction.initiators.indexOf(initiator) >= index,
          );
          ...
          interaction.initiators.forEach(function (initiator) {
            ...
            const report = reportsService.lookupPersonReport(interaction.organization.id, initiator.id);
          ...
          const report = reportsService.lookupOrganizationReport(interaction.organization.id);
          reportsService.incrementReportInteraction(report, interaction.interaction_type_id);
    ...
    updateInteraction: function (interaction) {
      return httpProxy
        .put(modelsService.getModelMetadata('interaction').url.single(interaction.id), interaction.serialize(), {
