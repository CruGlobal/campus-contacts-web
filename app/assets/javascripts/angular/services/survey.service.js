function surveyService(envService, $q, $http, httpProxy, modelsService, loggedInPerson, periodService, $log) {
  return {
    ...
    getReusableQuestions: (orgId, currentQuestions) => {
      return httpProxy
        ...
        .then(({ data }) => {
          return [
            ...new Set(data.active_survey_elements.map((element) => element.question)), // Use set to remove duplicate questions found in separate surveys
          ]
            .filter(
              (question) =>
            ...
            .reduce(
              ({ predefined, previouslyUsed }, question) => ({
                predefined: [...predefined, ...(question.predefined ? [question] : [])],
                previouslyUsed: [...previouslyUsed, ...(question.predefined ? [] : [question])],
              }),
              {
                predefined: [],
                previouslyUsed: [],
    ...
    createSurvey: (title, organization) => {
      ...
      return httpProxy
        .post(modelsService.getModelMetadata('survey').url.all, payload, {
    ...
    createSurveyQuestion: (surveyId, attributes) => {
      ...
      return isPredefinedQuestion
        ? httpProxy
            .put(`/surveys/${surveyId}/questions/${attributes.id}`, payload, {
              errorMessage: 'surveyTab:errors.addSurveyQuestion',
            })
            .then((survey) => {
              return survey.data;
            })
    ...
    updateSurveyQuestion: (surveyId, attributes, rules) => {
      ...
      return httpProxy.put(`/surveys/${surveyId}/questions/${attributes.id}`, payload, {
        errorMessage: 'surveyTab:errors.createSurvey',
      });
    },

    deleteSurveyQuestion: (surveyId, questionId) => {
      return httpProxy.delete(`/surveys/${surveyId}/questions/${questionId}`, null, {
        errorMessage: 'surveyTab:errors.createSurvey',
      });
    },

    deleteSurveyQuestionRule: (surveyId, ruleId) => {
      return httpProxy.delete(`/surveys/${surveyId}/question_rules/${ruleId}`, null, {
        errorMessage: 'surveyTab:errors.createSurvey',
      });
    },
          ...
          modelsService.getModelMetadata('survey').url.single(survey.id),
          {
            data: {
              ...
              attributes: {
                ...
                validate_phone_number: survey.validate_phone_number,
                validation_message: survey.validation_message,
                validation_success_message: survey.validation_success_message,
                ...(survey.logo === undefined ? {} : { logo: survey.logo }), // Only send logo field to API when it is a base64 string or null (to allow deletion)
              },
            },
          },
          {
    ...
    deleteSurvey: (survey) => {
      return httpProxy.delete(modelsService.getModelMetadata('survey').url.single(survey.id), null, {
    ...
    deleteKeyword: (keywordId) => {
      return httpProxy.delete(modelsService.getModelMetadata('sms_keyword').url.single(keywordId), null, {
    ...
    requestKeyword: (data) => {
      ...
      return httpProxy
        .post(modelsService.getModelMetadata('sms_keyword').url.all, payload, {
    ...
    updateKeyword: (data) => {
      ...
      return httpProxy
        .put(modelsService.getModelMetadata('sms_keyword').url.single(data.keyword.id), payload)
        .then(function (keyword) {
          return keyword.data;
        });
    },
    ...
    importContacts: (payload) => {
      return httpProxy
        .post(modelsService.getModelMetadata('bulk_create_job').url.all, payload, {
    ...
    importAnswerSheet: (payload) => {
      return httpProxy
        .post(modelsService.getModelMetadata('answer_sheet').url.all, payload, {
