...
describe('groupsService', function() {
    ...
    beforeEach(inject(function(
        ...
        function generateMembership(attributes) {
            var membership = new JsonApiDataStore.Model(
                'group_membership',
                attributes.id,
            );
            membership.setRelationship(
                'person',
                new JsonApiDataStore.Model('person', attributes.person_id),
            );
        ...
        it('should return null if the person is not a member', function() {
            expect(
                groupsService.findMember(
                    this.group,
                    new JsonApiDataStore.Model('person', 123),
                ),
            ).toBe(null);
        });
    });
    ...
    describe('payloadFromGroup', function() {
        it('should generate a JSON API payload', function() {
            ...
            var groupMembership1 = new JsonApiDataStore.Model(
                'group_membership',
                1,
            );
            ...
            var groupMembership2 = new JsonApiDataStore.Model(
                'group_membership',
                2,
            );
            ...
            expect(
                groupsService.payloadFromGroup(this.group, newLeaders),
            ).toEqual({
                data: {
                    type: 'group',
                    id: 3,
