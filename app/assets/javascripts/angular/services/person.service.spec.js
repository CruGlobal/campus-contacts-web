...
// Constants
let personService, $rootScope, httpProxy;
...
describe('personService', function () {
  ...
  describe('getOrgPermission', function () {
    ...
    it('should set the permission_id to be a string', function () {
      expect(personService.getOrgPermission(this.person, 1).permission_id).toEqual('4');
    });
  ...
  describe('isContact', function () {
    it('should return true for contacts', function () {
      this.person.organizational_permissions = [{ organization_id: 1, permission_id: 2 }];
      expect(personService.isContact(this.person, 1)).toBe(true);
    });

    it('should return false for users', function () {
      this.person.organizational_permissions = [{ organization_id: 1, permission_id: 4 }];
      expect(personService.isContact(this.person, 1)).toBe(false);
    });

    it('should return false for admins', function () {
      this.person.organizational_permissions = [{ organization_id: 1, permission_id: 1 }];
      expect(personService.isContact(this.person, 1)).toBe(false);
    });
  ...
  describe('getAssignedTo', function () {
    it("should find the person's assignments", function () {
      expect(personService.getAssignedTo(this.person, 1)).toEqual([{ name: 'Alice' }, { name: 'Adam' }]);
  ...
  describe('getContactAssignments', function () {
    beforeEach(function () {
      ...
      this.personRelevant = {
        ...
        organizational_permissions: [{ organization_id: this.organizationId }],
      };
    ...
    describe('scoped to organization', function () {
      ...
      it(
        ...
        asynchronous(function () {
          ...
          return personService.getContactAssignments(this.person, this.organizationId).then(function (assignments) {
    ...
    describe('not scoped to organization', function () {
      ...
      it(
        ...
        asynchronous(function () {
          ...
          return personService.getContactAssignments(this.person).then(function (assignments) {
            expect(assignments).toEqual([_this.personRelevant.reverse_contact_assignments[0]]);
          });
        }),
      );
