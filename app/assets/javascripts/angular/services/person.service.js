...
function personService(httpProxy, modelsService, JsonApiDataStore, permissionService, _, $log) {
  ...
  var personService = {
    ...
    getNewPerson: function (orgId) {
      const orgPermission = new JsonApiDataStore.Model('organizational_permission');
      ...
      const person = new JsonApiDataStore.Model('person', null);
    ...
    isContact: function (person, organizationId) {
      const orgPermission = personService.getOrgPermission(person, organizationId);
    ...
    getFollowupStatusOptions: function () {
      return generateOptionsFromIds(
        ['attempted_contact', 'completed', 'contacted', 'do_not_contact', 'uncontacted'],
        'followup_status',
      );
    },
    ...
    getCruStatusOptions: function () {
      return generateOptionsFromIds(
        ['none', 'volunteer', 'affiliate', 'intern', 'part_time_staff', 'full_time_staff'],
        'cru_status',
      );
    },
    ...
    getEnrollmentOptions: function () {
      return generateOptionsFromIds(
        ['not_student', 'middle_school', 'high_school', 'collegiate', 'masters_or_doctorate'],
        'student_status',
      );
    },
    ...
    archivePerson: function (person, organizationId) {
      const organizationalPermission = personService.getOrgPermission(person, organizationId);
      const updateJson = {
        data: {
          type: 'person',
          attributes: {},
    ...
    getContactAssignments: function (person, organizationId, additionalIncludes) {
      let include = additionalIncludes || [];

      // When we have an organization, we do not need to load the contact assignments' organizations
      include = include.concat(
        _.isNil(organizationId) ? 'reverse_contact_assignments.organization' : 'reverse_contact_assignments',
      ...
      return httpProxy
        ...
        .then(function (response) {
          ...
          try {
            ...
            $log.warn('Encounterd error while logging people loading limit', e);
          }
