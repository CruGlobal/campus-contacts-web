...
// Constants
let organizationService, $rootScope, $q, JsonApiDataStore, _;
  ...
  describe('getOrgHierarchy', function () {
    it(
      ...
      asynchronous(function () {
        spyOn(organizationService, 'loadOrgsById').and.returnValue($q.resolve([]));
        return organizationService.getOrgHierarchy(this.orgs[3]).then(function (orgs) {
          expect(_.map(orgs, 'id')).toEqual(['1', '2', '3']);
          expect(organizationService.loadOrgsById).toHaveBeenCalledWith([], jasmine.any(String));
        });
      }),
    );
    ...
    it(
      ...
      asynchronous(function () {
        const _this = this;
        spyOn(organizationService, 'loadOrgsById').and.callFake(function () {
        ...
        return organizationService.getOrgHierarchy(this.orgs[5]).then(function (orgs) {
          ...
          expect(organizationService.loadOrgsById).toHaveBeenCalledWith(['4'], jasmine.any(String));
        });
      }),
    );
