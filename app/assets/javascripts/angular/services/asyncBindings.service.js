angular.module('campusContactsApp').factory('asyncBindingsService', asyncBindingsService);
...
function asyncBindingsService($injector, $q, _) {
  return {
    ...
    lazyLoadedResolve: function (resolve) {
      const dependencies = $injector.annotate(resolve);
      return dependencies.concat([
        function () {
          // Extract the resolve function if the resolve uses inline array annotation
          const resolveFn = _.isArray(resolve) ? _.last(resolve) : resolve;
    ...
    lazyLoadedActivate: function (activate, bindingNames) {
      return function preactivate() {
        ...
        const resolvePromises = bindingNames.map(function (bindingName) {
          // If the resolve was decorated by lazyLoadedResolve, then we need to unwrap the binding value
          // to get the real value
          const bindingValue = vm[bindingName];
          const unwrappedValue = bindingValue && bindingValue.wrapped ? bindingValue.wrapped : bindingValue;
