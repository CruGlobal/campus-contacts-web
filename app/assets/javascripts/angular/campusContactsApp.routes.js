angular
  ...
  .config(function (
    ...
    const asyncBindingsService = asyncBindingsServiceProvider.$get();

    // Convenience alias
    const lazyLoadedResolve = asyncBindingsService.lazyLoadedResolve;
    ...
    $stateProvider.state = function (state) {
      const transform = state.modal ? modalState : _.identity;
    ...
    function modalState(state) {
      ...
      return {
        ...
        onEnter: /* @ngInject */ function ($state, $uibModal) {
          ...
          const injectedProps = _.keys(state.resolve);
          const attributes = injectedProps.map(function (property) {
            return _.kebabCase(property) + '="$ctrl.' + property + '"';
          });
          const directive = _.kebabCase(state.component);
          const template = '<' + directive + ' ' + attributes.join(' ') + '></' + directive + '>';
          ...
          modalInstance = $uibModal.open({
            ...
            controller: injectedProps.concat([
              function () {
                ...
                const injectedValues = arguments;
                const injections = _.zipObject(injectedProps, injectedValues);
      ...
      states.push({
        ...
        resolve: {
          ...
          person: lazyLoadedResolve(
            /* @ngInject */ function ($state, $uiRouter, routesService, personService) {
          ...
          organizationId: /* @ngInject */ ($uiRouter) => {
            const $transition$ = $uiRouter.globals.transition;
            return $transition$.params().orgId ? $transition$.params().orgId : false;
          },
    ...
    $stateProvider
      ...
      .state({
        ...
        resolve: {
          ...
          organizationId: ($location) => $location.search().organization_id,
          organizationName: ($location) => $location.search().organization_name,
        },
      })
      .state({
      ...
      .state({
        ...
        resolve: {
          rememberCode: ($transition$) => $transition$.params().rememberCode,
      ...
      .state({
        ...
        resolve: {
          userId: ($state, $transition$) => $transition$.params().userId,
        },
      })
      .state({
      ...
      .state({
        ...
        url: '/admin',
        resolve: {
          adminRedirect: (authenticationService) => authenticationService.adminRedirect(),
        },
      })
      .state({
        name: 'app.people',
        url: '/people',
        template: '<my-people-dashboard edit-mode="$ctrl.editOrganizations"></my-people-dashboard>',
      })
      .states(
      ...
      .state({
        ...
        url: '/ministries',
        abstract: true,
        template: '<my-organizations-dashboard></my-organizations-dashboard>',
      })
      .state({
        ...
        resolve: {
          rootOrgs: function (myOrganizationsDashboardService, userPreferencesService, $q, $state) {
            return myOrganizationsDashboardService
              ...
              .then(userPreferencesService.applyUserOrgDisplayPreferences)
              .then((orgs) => {
                if (orgs.length === 1) {
      ...
      .state({
        ...
        resolve: {
          surveyId: ($state, $transition$) => $transition$.params().surveyId,
        },
      })
      .state({
      ...
      .states(
        ministryViewTabs.map(function (tab) {
          const formatArrayForFilter = (filter) => {
            ...
            return filter.split(',').reduce((acc, value) => ({ ...acc, [value]: true }), {});
          };
          ...
          if (tab === 'people') {
            return {
              ...
              resolve: {
                queryFilters: /* @ngInject */ ($transition$, $location) => {
                  const filters = $location.search();

                  return {
                    ...(filters.assigned_to
                      ? {
                          assigned_tos: formatArrayForFilter(filters.assigned_to),
                        }
                      : {}),
                    ...(filters.statuses
                      ? {
                          statuses: formatArrayForFilter(filters.statuses),
                        }
                      : {}),
                  };
                },
      ...
      .state({
        ...
        resolve: {
          phoneNumberValidation: ($state, $transition$, routesService) => {
            return routesService
              .getPhoneNumberValidation($transition$.params().code, $transition$.params().id)
              .catch((e) => null);
          },
        },
      })
      .state({
        ...
        resolve: {
          survey: ($state, $transition$, routesService) => {
            return routesService.getSurvey($transition$.params().surveyId).catch((e) => null);
          },
          preview: ($state, $transition$) => !!$transition$.params().preview,
        },
      })
      .state({
        ...
        component: 'publicSurvey',
        resolve: {
          survey: ($state, $transition$, routesService) => routesService.getSurvey($transition$.params().surveyId),
          preview: () => true,
        },
      })
      .state({
      ...
      .state({
        ...
        resolve: {
          survey: ($state, $transition$, routesService) => {
            return routesService.getSurvey($transition$.params().surveyId);
          },
        },
      })
      .state({
