...
function surveyOverviewSettingsController(surveyService, confirmModalService, tFilter, loggedInPerson) {
  ...
  this.selectImage = () => {
    ...
    input.addEventListener(
      ...
      () => {
        ...
        // validate type
        if (!_.includes(['image/jpeg', 'image/png'], this.selectedImage.type)) {
          return;
        }
  ...
  this.deleteImage = () => {
    ...
    confirmModalService.create(tFilter('surveys:settings:image_delete_confirm')).then(() => {
      this.surveyEdit.logo_url = null;
