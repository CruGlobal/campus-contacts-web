angular.module('campusContactsApp').factory('organizationOverviewSuborgsService', organizationOverviewSuborgsService);
...
function organizationOverviewSuborgsService(httpProxy, modelsService) {
  var organizationOverviewSuborgsService = {
    ...
    loadOrgSubOrgs: function (org, listLoader) {
      const requestParams = organizationOverviewSuborgsService.buildGetParams(org.id || org);
      if (listLoader) {
        return listLoader.loadMore(requestParams);
      }

      return httpProxy.get(modelsService.getModelMetadata('organization').url.all, requestParams).then((resp) => {
        return resp.data;
      });
    },

    loadOrgSubOrgCount: function (orgId) {
      const requestParams = organizationOverviewSuborgsService.buildGetParams(orgId);
