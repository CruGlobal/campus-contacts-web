angular.module('campusContactsApp').factory('personPageService', personPageService);
...
function personPageService(httpProxy, modelsService, _) {
  return {
    ...
    uploadAvatar: function (person, file) {
      const url = modelsService.getModelUrl(person);
      const form = {
        data: {
          attributes: {
            picture: file,
    ...
    savePerson: function (person) {
      const attributeNames = ['first_name', 'last_name', 'gender', 'student_status'];
      ...
      const personAttributes = person.serialize({
        attributes: attributeNames,
        relationships: [],
      }).data;

      const includedRelationships = _.chain(relationshipNames)
