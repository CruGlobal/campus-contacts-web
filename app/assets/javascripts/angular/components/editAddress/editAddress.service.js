angular.module('campusContactsApp').factory('editAddressService', editAddressService);
...
function editAddressService(geoDataService, JsonApiDataStore, _) {
  var editAddressService = {
    ...
    getAddressTemplate: function (person) {
      ...
      const addressType = _.chain(editAddressService.getAddressTypes())
        .difference(_.map(person.addresses, 'address_type'))
        .first()
        .defaultTo(null)
        .value();

      const address = new JsonApiDataStore.Model('address');
      address.setAttribute('person_id', person.id);
      address.setAttribute('address_type', addressType);
      ['address1', 'address2', 'address3', 'address4', 'city', 'state', 'zip', 'country'].forEach(function (attribute) {
    ...
    getCountryOptions: function () {
      return geoDataService.getCountries().then(function (countries) {
        ...
        const priorityCountryCodes = ['US', 'CA'];
        const priorityCountries = priorityCountryCodes.map(function (countryCode) {
    ...
    getRegionOptions: function (countryCode) {
      return geoDataService.getCountries().then(function (countries) {
        const country = _.find(countries, { shortCode: countryCode });
