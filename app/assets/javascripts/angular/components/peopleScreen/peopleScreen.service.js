angular.module('campusContactsApp').factory('peopleScreenService', peopleScreenService);
...
function peopleScreenService(
  ...
  const peopleScreenService = {
    ...
    loadAssignedTos: function (people, orgId) {
      ...
      const unloadedIds = _.chain(assignedTos).filter(_.negate(httpProxy.isLoaded)).map('id').value();
    ...
    loadMoreOrgPeople: function (orgId, filters, order, listLoader, loaderService, surveyId) {
      const requestParams = peopleScreenService.buildListParams(orgId, filters, order, surveyId);

      return listLoader
        .loadMore(requestParams, surveyId ? peopleScreenService.transformData : undefined)
        .then(function (resp) {
    ...
    buildListParams: (orgId, filters = {}, orderParam = [], surveyId = '') => ({
      include: [
        'phone_numbers',
        'email_addresses',
      ...
      'fields[organizational_permission]': 'followup_status,organization_id,permission_id,archive_date',
    ...
    buildOrderString: function (order) {
      return order
        .map(function (orderEntry) {
          return `${orderEntry.direction === 'desc' ? '-' : ''}${orderEntry.field}`;
        })
        .join(',');
    },
    ...
    transformData: (params) => (data) => {
      ...
      return Object.assign(data, {
        answers: (
          _.findLast(
            data.answer_sheets,
            (sheet) => sheet.survey.id === params['filters[answer_sheets][survey_ids]'],
          ) || { answers: [] }
        ).answers.reduce(
          (acc, answer) => ({
            ...acc,
            [answer.question.id]: [...(acc[answer.question.id] || []), answer.value],
          }),
          {},
        ),
      });
    ...
    exportPeople: function (orgId, selection, order, surveyId) {
      const filterParams = peopleScreenService.buildListParams(orgId, selection.filters, order, surveyId);
      ...
      const url = envService.read('apiUrl') + modelsService.getModelMetadata('person').url.all + '?' + queryString;
