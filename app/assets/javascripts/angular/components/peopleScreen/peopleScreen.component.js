...
function peopleScreenController(
  ...
  this.columns = [
    ...
    },
    {
      ...
      sortable: true,
      getSortKey: (person) => {
        const orgPermission = personService.getOrgPermission(person, this.org.id);

        if (!orgPermission || orgPermission.archive_date !== null) return 'Archived';

        return personService.getFollowupStatus(person, this.org.id);
      },
      orderFields: ['organizational_permissions.followup_status', 'last_name', 'first_name'],
    },
    {
      name: 'contactActions',
  ...
  const addLastSurveyColumn = () => {
    this.columns.push({
      ...
      orderFields: ['answer_sheets.updated_at', 'last_name', 'first_name'],
    });
  };
  ...
  this.loadPersonPage = () => {
    ...
    return peopleScreenService
      .loadMoreOrgPeople(orgId, this.filters, getOrder(), listLoader, this.loaderService, this.surveyId)
      .then((resp) => {
    ...
    $uibModal
      ...
      .result.then(() => {
        ...
        if (!peopleFiltersPanelService.filtersHasActive(this.filters)) {
          ...
          peopleScreenService.loadAssignedTos(this.people, this.org.id);
          return;
        }
        ...
        this.loadPersonPage().then(() => {
          ...
          const relevantOriginalSelections = _.pick(originalMultiSelection, _.map(this.people, 'id'));
          ...
          this.selectAllValue = originalSelectAllValue && getSelectedPeople().length > 0;
        });
      });
  };
  ...
  this.exportPeople = () => {
    peopleScreenService.exportPeople(this.org.id, getSelection(), getOrder(), this.surveyId);
  };
  ...
  this.archivePeople = () => {
    removePeople('ministries.people.archive_people_confirm', peopleScreenService.archivePeople);
  };

  this.deletePeople = () => {
    removePeople('ministries.people.delete_people_confirm', peopleScreenService.deletePeople);
  };
