...
function organizationSignaturesController(httpProxy, $scope, $window, envService, $http) {
  this.signatures = [];
  this.signatureData = [];
  this.exportLink = '';
  ...
  const buildExportCsvLink = (searchText) => {
    ...
    const query = Object.entries(params).reduce((accumulator, [key, value]) => {
      return `${accumulator}&${encodeURIComponent(key)}=${encodeURIComponent(value)}`;
    }, '');

    return envService.read('apiUrl') + `/organizations/${this.orgId}/signatures?${query}`;
  };
  ...
  const loadData = async (tableState, searchText) => {
    ...
    const { data, meta } = await getSignatures(this.orgId, searchText, start);
    ...
    tableState.pagination.currentPage = Math.ceil(start / this.itemsPerPage) + 1;
  ...
  this.nextPage = (searchText) => {
    this.tableState.pagination.start = this.tableState.pagination.currentPage * this.itemsPerPage;
    loadData(this.tableState, searchText);
  };

  this.previousPage = (searchText) => {
    this.tableState.pagination.start = (this.tableState.pagination.currentPage - 2) * this.itemsPerPage;
    loadData(this.tableState, searchText);
  };
