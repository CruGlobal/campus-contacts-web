...
function organizationOverviewController(
  ...
  this.isTabVisible = (tabName) => {
    return tabName !== 'labels' && !(tabName === 'surveys' && !this.directAdminPrivileges);
  };
  ...
  function activate() {
    ...
    organizationOverviewService.getSubOrgCount(this.org).then((subOrgCount) => {
      this.suborgs = new Array(subOrgCount);
    });
    organizationOverviewService.getPersonCount(this.org).then((personCount) => {
      this.people = new Array(personCount);
    });
    organizationOverviewService.getTeamCount(this.org).then((teamMemberCount) => {
