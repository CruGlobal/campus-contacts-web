...
function orgManagementSubOrgController(organizationOverviewSuborgsService, organizationService, $uibModal, $timeout) {
  ...
  this.$onInit = () => {
    if (this.isRoot) {
      ...
      organizationOverviewSuborgsService.loadOrgSubOrgs(this.org).then((subOrgs) => {
    ...
    $uibModal
      .open({
        ...
        resolve: {
          icon: () => warningIcon,
          title: () =>
            i18next.t('ministries:organizations.deleteConfirmModal.title', {
              org_name: org.name,
            }),
          paragraphs: () => [i18next.t('ministries:organizations.deleteConfirmModal.description')],
