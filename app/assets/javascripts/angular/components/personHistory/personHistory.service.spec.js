...
describe('personHistoryService', function () {
  ...
  describe('buildHistoryFeed', function () {
    ...
    it('should show just interactions', function () {
      expect(personHistoryService.buildHistoryFeed(this.person, 'interactions', 1)).toEqual([this.interaction]);
    });

    it('should show just notes', function () {
      expect(personHistoryService.buildHistoryFeed(this.person, 'notes', 1)).toEqual([this.note]);
    });

    it('should show just surveys', function () {
      expect(personHistoryService.buildHistoryFeed(this.person, 'surveys', 1)).toEqual([
        this.answerSheet2,
        this.answerSheet1,
      ]);
    });
    ...
    it('should filter out history from other orgs', function () {
      ...
      expect(personHistoryService.buildHistoryFeed(this.person, 'all', 1)).toEqual([this.answerSheet1, this.note]);
    });
  });
});
