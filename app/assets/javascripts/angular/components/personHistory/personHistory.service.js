angular.module('campusContactsApp').factory('personHistoryService', personHistoryService);
...
function personHistoryService(_) {
  return {
    ...
    buildHistoryFeed: function (person, feedFilter, orgId) {
      return _.sortBy(filterHistoryByType(person, feedFilter, orgId), getHistorySortKey);
    },
  };
  ...
  function filterHistoryByType(person, feedFilter, orgId) {
    switch (feedFilter) {
      case 'interactions':
        return filterInteractionByOrg(person.interactions, orgId).filter(function (interaction) {
          return interaction.interaction_type_id !== '1';
        });
      case 'notes':
        return _.filter(
          filterInteractionByOrg(person.interactions, orgId),
          ({ interaction_type_id }) => `${interaction_type_id}` === '1',
        );
      case 'surveys':
