...
angular.module('campusContactsApp').component('organizationContactImportStep2', {
  bindings: {
    next: '&',
    previous: '&',
    csvData: '<',
...
function organizationContactImportStep2Controller($scope, surveyService) {
  ...
  this.$onInit = () => {
    ...
    surveyService.getSurveyQuestions(this.selectedSurvey.id).then((questions) => {
  ...
  this.nextPreviewRow = () => {
    this.currentPreviewRow =
      this.currentPreviewRow + 1 < this.csvData.length ? this.currentPreviewRow + 1 : this.currentPreviewRow;
  };

  this.previousPreviewRow = () => {
    this.currentPreviewRow = this.currentPreviewRow - 1 > 0 ? this.currentPreviewRow - 1 : this.currentPreviewRow;
  };
}
