...
angular.module('campusContactsApp').component('organizationContactImportStep3', {
  bindings: {
    org: '<',
    next: '&',
    previous: '&',
...
function organizationContactImportStep3Controller($scope, surveyService, labelsService) {
  this.contactLabels = {};
  ...
  this.addLabel = (labelName) => {
    const label = labelsService.getLabelTemplate(this.selectedSurvey.organization_id);
  ...
  this.import = () => {
    ...
    surveyService.importContacts(postData).then(
      ...
      (response) => {
        const { data: { errors: [{ detail: { bulk_data } = [] } = {}] = [] } = {} } = response;
