...
function copySurveyController(surveyService, loggedInPerson) {
  ...
  this.$onInit = () => {
    ...
    this.orgs = loggedInPerson.person.organizational_permissions.map((orgPermission) => orgPermission.organization);
  ...
  this.save = () => {
    ...
    surveyService.copySurvey(this.survey.id, this.survey.parent_organization, this.survey.title).then(
      (newSurvey) => {
