...
angular.module('campusContactsApp').component('reportMovementIndicatorsSuggestedActions', {
...
function reportMovementIndicatorsSuggestedActionsController(httpProxy) {
  ...
  const loadSuggestedActions = (orgId) => {
    ...
    return httpProxy
      .post(
        ...
        {
          errorMessage: i18next.t('movementIndicators:suggestedActions.errorLoadingSuggestedActions'),
          params: {
            include: 'person',
            'fields[movement_indicator_suggestion]': 'id,person,reason,label,considered',
      ...
      .then(({ data }) => {
        ...
        const { newSuggestions, previousSuggestions } = data.reduce(
          (acc, action) => ({
            newSuggestions: [...acc.newSuggestions, ...(action.considered ? [] : [action])],
            previousSuggestions: [...acc.previousSuggestions, ...(action.considered ? [action] : [])],
          }),
  ...
  this.saveSuggestedActions = () => {
    return httpProxy
      .post(
        ...
        {
          errorMessage: i18next.t('movementIndicators:suggestedActions.errorSavingSuggestedActions'),
        },
      )
      .then(() => {
        this.next();
  ...
  this.handleAllClick = (suggestions) => {
    this.acceptedIds = new Set([...this.acceptedIds, ...suggestions.map(({ id }) => id)]);
    this.rejectedIds = new Set([...this.rejectedIds].filter((id) => !this.acceptedIds.has(id)));
  };
}
