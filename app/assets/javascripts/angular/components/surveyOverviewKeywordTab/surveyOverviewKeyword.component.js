...
function surveyOverviewKeywordController(
  ...
  const keywordErrorHandler = (response) => {
    let { data: { errors: [{ detail: details } = {}] = [] } = {} } = response;
  ...
  this.$onInit = () => {
    ...
    this.disableKeywordField = this.keyword.keyword && _.includes(['requested', 'active'], this.keyword.state);
  };

  this.requestKeyword = () => {
    this.keywordError = '';
    if (!this.keyword.keyword) {
      this.keywordError = tFilter('surveys:keyword:errors:missingKeyword');
    } else if (!this.keyword.explanation) {
      this.keywordError = tFilter('surveys:keyword:errors:missingPurpose');
    } else if (!this.keyword.initial_response) {
      this.keywordError = tFilter('surveys:keyword:errors:missingTextResponse');
    }
      ...
      .then((keywordData) => {
        ...
        $uibModal.open({
          ...
          resolve: {
            ...
            paragraphs: () => [i18next.t('surveys:keyword.requested_message')],
  ...
  this.deleteKeyword = (keywordId) => {
    ...
    confirmModalService.create(tFilter('surveys:keyword:delete:confirm')).then(() => {
      this.survey.keyword = {};
      this.keyword = {};
