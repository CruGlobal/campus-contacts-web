...
function editGroupOrLabelAssignmentsService($q, httpProxy, modelsService, JsonApiDataStore, personProfileService, _) {
  function pushNew(modelType, person, newIds, organizationId) {
    const relationships = _.map(newIds, function (labelId) {
  ...
  function buildRelationshipModel(modelType, relatedId, organizationId) {
    const model = new JsonApiDataStore.Model(modelType);
  ...
  function personRelationshipName(modelType) {
    ...
    return modelType === 'organizational_label' ? 'organizational_labels' : 'group_memberships';
  }
  ...
  function deleteByRelatedId(type, relatedId, person) {
    const relationshipName = personRelationshipName(type);
    const relatedType = relatedModel(type);
    const relationship = _.find(person[relationshipName], [relatedType + '.id', relatedId]);
    return personProfileService.deleteRelationship(person, relationship, relationshipName);
  }

  function saveRelationshipChanges(type, person, addedRelationshipIds, removedRelationshipIds, organizationId) {
    const promises = _.map(removedRelationshipIds, function (relatedId) {
  ...
  function updateGroupMemberships(person, addedGroupIds, removedGroupIds) {
    _.each(addedGroupIds, function (groupId) {
      const group = JsonApiDataStore.store.find('group', groupId);
      const membership = _.find(person.group_memberships, ['group.id', groupId]);
      group.group_memberships.push(membership);
    });
    _.each(removedGroupIds, function (groupId) {
      const group = JsonApiDataStore.store.find('group', groupId);
      const membership = _.find(group.group_memberships, ['person.id', person.id]);
  ...
  return {
    saveOrganizationalLabels: function (person, organizationId, addedLabelIds, removedLabelIds) {
      return saveRelationshipChanges('organizational_label', person, addedLabelIds, removedLabelIds, organizationId);
    },

    saveGroupMemberships: function (person, addedGroupIds, removedGroupIds) {
      const savePromise = saveRelationshipChanges('group_membership', person, addedGroupIds, removedGroupIds);
    ...
    loadPlaceholderEntries: function (entries, orgId, errorMessage) {
      if (_.find(entries, { _placeHolder: true })) {
        httpProxy.get(
          modelsService.getModelMetadata('organization').url.single(orgId),
          {
