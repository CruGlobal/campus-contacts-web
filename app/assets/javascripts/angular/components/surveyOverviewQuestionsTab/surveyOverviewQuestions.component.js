...
function surveyOverviewQuestionsController($uibModal, surveyService, $scope, httpProxy, loggedInPerson) {
  this.isExpanded = {};
  ...
  const getLabel = async (questions, organizationId) => {
      ...
      ...new Set(
        questions.reduce((a1, q) => {
          const ids = q.question_rules.reduce((a2, r) => {
            const labelIds = r.label_ids ? r.label_ids.split(',') : [];
            return [...a2, ...labelIds];
          }, []);
          return [...a1, ...ids];
        }, []),
    ...
    const currentLabels = orgLabels.filter((label) => labelIds.includes(label.id));
  ...
  const getPeople = async (questions, organizationId) => {
    const peopleIds = [
      ...new Set(
        questions.reduce((a1, q) => {
          const ids = q.question_rules.reduce((a2, r) => {
            const peopleIds = r.people_ids ? r.people_ids.split(',') : [];
            return [...a2, ...peopleIds];
          }, []);
          return [...a1, ...ids];
        }, []),
    ...
    const { data } = await httpProxy.get(
      '/people',
      {
        ...
        'fields[people]': 'first_name,gender,last_name,primary_email_address,id',
        include: '',
      },
      {
        errorMessage: 'error.messages.surveys.loadQuestions',
  ...
  const buildQuestion = (question) => {
    ...
    if (a) {
      const autoassignRules = buildRules(question.question_rules, a, 'AUTOASSIGN');
      const autoNotifyRules = buildRules(question.question_rules, a, 'AUTONOTIFY');
  ...
  const buildRules = (questionRules, answerQuestion, type) => {
    return answerQuestion.map((answer) => buildRule(answer, questionRules, type));
  };

  const buildRule = (answer, questionRules, type) => {
    const { id = null, label_ids = null, organization_ids = null, people_ids = null } =
      questionRules.find((r) => r.trigger_keywords === answer && r.rule_code === type) || {};
    const ids = people_ids ? people_ids.split(',') : [];
    const labelIds = label_ids ? label_ids.split(',') : [];
    ...
    return {
      ...
      assigned_labels: id ? this.labels.filter((p) => labelIds.indexOf(p.id) >= 0) : null,
      assign_to: id ? this.people.filter((p) => ids.indexOf(p.id) >= 0) : null,
    };
  };

  const buildRulesWithChangedAnswer = (question, originalAnswer, newAnswer) => {
  ...
  this.$onInit = () => {
    this.directAdminPrivileges = loggedInPerson.isDirectAdminAt(this.survey.organization);

    loadSurveyData();
  };

  this.addLabelToRule = async (question, rule) => {
    const index = question.question_rules.findIndex((currentRule) => currentRule.id === rule.id);
    ...
    const ids = [
      ...new Set(question.question_rules[index].assigned_labels.filter((a) => a._type === 'label').map((b) => b.id)),
    ];
  ...
  this.addPersonToRule = async (question, rule) => {
    ...
    const ids = [
      ...new Set(question.question_rules[index].assign_to.filter((a) => a._type === 'person').map((b) => b.id)),
    ];
  ...
  this.getQuestionType = (kind, style) => {
    return _.find(this.questionTypes, { kind, style });
  };
  ...
  this.addPredefinedQuestion = () => {
    $uibModal.open({
      ...
      resolve: {
        ...
        currentQuestions: () => this.surveyQuestions.map(({ id }) => id),
  ...
  this.addQuestion = (question) => {
    ...
    surveyService.createSurveyQuestion(this.survey.id, question).then((newQuestion) => {
  ...
  this.deleteQuestion = (questionId) => {
    ...
    $uibModal
      .open({
        ...
        resolve: {
          icon: () => warningIcon,
          paragraphs: () => [i18next.t('surveys:questions.delete_confirm')],
          dismissLabel: () => i18next.t('cancel'),
          closeLabel: () => i18next.t('general.delete'),
        },
      })
      .result.then(() => {
        surveyService.deleteSurveyQuestion(this.survey.id, questionId).then(() => {
  ...
  this.saveQuestion = (question) => {
    ...
    const { id, label, kind, style, column_title, content, position } = question;
  ...
  this.addEmptyQuestionContent = (question) => {
    ...
    question.question_rules.push(buildRule('', question.question_rules, 'AUTOASSIGN'));
    question.question_rules.push(buildRule('', question.question_rules, 'AUTONOTIFY'));
  };
  ...
  this.deleteQuestionContent = async (question, answers, index) => {
    ...
    const ruleIdsToDelete = question.question_rules.reduce((accumulator, r) => {
  ...
  this.updateQuestionAnswer = async (question, answerIndex) => {
    ...
    const originalAnswer = oldAnswers.find((a) => question.question_answers.indexOf(a) === -1);

    question.question_rules = buildRulesWithChangedAnswer(question, originalAnswer, newAnswer);
  ...
  this.saveQuestionContent = async (question, answers) => {
    ...
    if (data.question_rules) {
      const index = this.surveyQuestions.findIndex((currentQuestion) => currentQuestion.id === question.id);
  ...
  this.openAutoAssignLabelNotifyModal = (question, answer) => {
    $uibModal.open({
      ...
      resolve: {
        ...
        question: () => question,
        answer: () => answer,
        addPersonToRule: () => (question, rule) => this.addPersonToRule(question, rule),
        addLabelToRule: () => (question, rule) => this.addLabelToRule(question, rule),
      },
    });
  };
}
