...
function predefinedQuestionsModalController(surveyService) {
  this.$onInit = () => {
    surveyService
      .getReusableQuestions(this.resolve.orgId, this.resolve.currentQuestions)
  ...
  this.addQuestion = (question) => {
    ...
    if (question.predefined) {
      this.predefinedQuestions.splice(this.predefinedQuestions.indexOf(question), 1);
    } else {
      this.previouslyUsedQuestions.splice(this.previouslyUsedQuestions.indexOf(question), 1);
    }
  };
}
