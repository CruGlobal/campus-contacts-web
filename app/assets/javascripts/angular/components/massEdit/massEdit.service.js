(function () {
    ...
    function massEditService ($q, httpProxy, modelsService, organizationOverviewTeamService, JsonApiDataStore, _) {
        ...
        function personHasLabel (person, orgId, label) {
            return Boolean(_.find(person.organizational_labels, function (organizationalLabel) {
        ...
        var personChangeFields = _.keys(_.pickBy(simpleChangeManifests, { type: 'person' }));

        // List of mass editable attributes that are on the organizational permission model
        var permissionChangeFields = _.keys(_.pickBy(simpleChangeManifests, { type: 'permission' }));
        ...
        var complexChangeManifests = {
            ...
            groups: {
                ...
                getModelAttributes: function (groupId, person, organizationId) { // eslint-disable-line no-unused-vars
                    return {
        ...
        var massEditService = {
            ...
            applyComplexChanges: function (changes, person, organizationId, manifest) {
                _.forEach(changes, function (change, id) {
                    var existingModel = _.find(person[manifest.personAttribute], manifest.getModelPredicate(id));

                    if (change === true && !existingModel) {
                        // Add a model
                        var addedAssignment = new JsonApiDataStore.Model(manifest.modelType);
            ...
            applyChangesLocally: function (selection, changes) {
                ...
                selection.selectedPeople.forEach(function (personId) {
                    ...
                    var orgPermission = _.find(person.organizational_permissions, { organization_id: orgId });
            ...
            statDefinitions: [
                { type: 'assigned_tos', idField: 'person_id', personHasModel: personHasAssignment },
                { type: 'labels', idField: 'label_id', personHasModel: personHasLabel },
                { type: 'groups', idField: 'group_id', personHasModel: personHasGroup }
            ],
            ...
            loadPeopleRelationships: function (selection) {
                ...
                var peopleMissingRelationships = selection.selectedPeople.filter(function (personId) {
                    var person = JsonApiDataStore.store.find('person', personId);
            ...
            optionsForStatsType: function (stats, statType, selection) {
                var statDefinition = _.find(massEditService.statDefinitions, { type: statType });
                return stats[statDefinition.type].map(function (stat) {
                    ...
                    if (stat.count === 0) {
                        ...
                        state = massEditService.optionStateFromModel({ id: id, count: stat.count }, selection,
            ...
            optionsFromStats: function (stats, selection) {
                ...
                return _.fromPairs(statTypes.map(function (statType) {
            ...
            optionStateFromModel: function (model, selection, personHasModel) {
                ...
                var selectedPeople = selection.selectedPeople.map(function (personId) {
