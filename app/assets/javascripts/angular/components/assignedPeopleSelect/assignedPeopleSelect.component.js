...
function assignedPeopleSelectController($scope, assignedPeopleSelectService, RequestDeduper) {
  this.people = [];
  ...
  this.$onInit = () => {
    ...
    $scope.$watch('$select.search', (search) => {
      assignedPeopleSelectService.searchPeople(search, this.organizationId, requestDeduper).then((people) => {
        if (this.assigned === null) {
          this.people = people;
        } else {
          const assignedPeopleIds = this.assigned.map(({ id }) => id);
          // Find all the current people in this.assigned
          this.people = people.filter((person) => !assignedPeopleIds.includes(person.id));
        }
      });
    });
