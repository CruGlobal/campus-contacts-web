class Permission < ActiveRecord::Base
  ...
  scope :default,
        -> { where(i18n: %w(admin user no_permissions)) } if Permission.table_exists? # added for travis testing
  scope :users,
        -> { where(i18n: %w(user admin)) }
  scope :default_permissions,
        -> { where("i18n IN #{default_permissions_for_field_string(self::DEFAULT_PERMISSIONS)}").order("FIELD#{i18n_field_plus_default_permissions_for_field_string(self::DEFAULT_PERMISSIONS)}") }
  scope :non_default_permissions,
        -> { where('i18n IS NULL').order('permissions.name ASC') }
  scope :arrange_all,
        -> { order("#{order_default_permissions}") }

  def self.order_default_permissions
    str = 'CASE '
    self::DEFAULT_PERMISSIONS.each_with_index do |p, i|
      str += "WHEN permissions.i18n = '#{p}' THEN #{i} "
    end
    str += 'ELSE permissions.id END'
  end
  ...
  def to_s
    %w(admin user no_permissions).include?(i18n) ? I18n.t("permissions.#{i18n}") : name
  end
  ...
  def self.i18n_field_plus_default_permissions_for_field_string(permissions)
    r = default_permissions_for_field_string(permissions)
    r[0] = ''
  ...
  DEFAULT_PERMISSIONS = %w(admin user no_permissions)
