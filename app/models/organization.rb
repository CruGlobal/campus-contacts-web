...
class Organization < ActiveRecord::Base
  ...
  def assigned_contacts_with_archived
    ...
    assignments.present? ? all_people_with_archived.where(id: assignments.collect(&:person_id)) : nil
  end
  ...
  def add_permission_to_person(person, permission_id, added_by_id = nil)
    ...
    if person.present? && permission = Permission.where(id: permission_id).first
      ...
      if org_permission.permission_id == permission.id
        ...
        unless new_org_permission = OrganizationalPermission.where(permission_id: permission.id, person_id: person.id, organization_id: id).try(:first).present?
