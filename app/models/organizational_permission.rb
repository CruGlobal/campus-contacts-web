...
class OrganizationalPermission < ActiveRecord::Base
  ...
  FOLLOWUP_STATUSES = %w(uncontacted attempted_contact contacted do_not_contact completed)
  ...
  def self.get_permission_hash(org, people)
    permission_hash = {}
    active.where(organization_id: org.id, person_id: people.collect(&:id)).each do |op|
  ...
  def check_if_only_remaining_admin_permission_in_a_root_org
    fail CannotDestroyPermissionError if permission_id == Permission::ADMIN_ID && organization.is_root_and_has_only_one_admin?
  end

  def check_if_admin_is_destroying_own_admin_permission(destroyer)
    fail CannotDestroyPermissionError if permission_id == Permission::ADMIN_ID && destroyer && person_id == destroyer.id
  end
  ...
  def notify_new_leader
    ...
    if person.blank? || person.user.blank?
      ...
      LeaderMailer.delay.added(person, added_by_id, token)
    end
  end
