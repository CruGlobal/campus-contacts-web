class PersonSignature < ActiveRecord::Base
  ...
  scope :filter, lambda { |search_name, org_ids = nil|
    ...
    if data.present?
      ...
      data = data.where('organizations.id IN (?)', org_ids) if org_ids.present?
    end
    data
  }

  scope :sort, lambda { |sort_query|
  ...
  scope :get_by_multiple_orgs, lambda { |org_ids|
  ...
  def has_signed_signature?(kind)
    signature = signatures.find_by(kind: kind)
