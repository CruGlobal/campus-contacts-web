class PersonFilter
  ...
  def filter(people)
        ...
        if survey = Survey.find_by(id: survey_id)
          ...
          if questions = survey_options['questions']
            questions.each do |question_id, keyword|
              if question = survey.questions.find_by(id: question_id.to_i)
    ...
    if @filters[:roles] && filtered_people.present?
      permission_ids = @filters[:roles].split(',').collect { |r| r.to_i > 0 ? r : Permission.find_by(name: r).try(:id) }.compact
      label_ids = @filters[:roles].split(',').collect { |r| r.to_i > 0 ? r : Label.find_by(name: r).try(:id) }.compact
    ...
    if @filters[:gender] && filtered_people.present?
      if @filters[:gender] == 'no_response'
        ...
        @filters[:gender].split(',').each do |gender|
          ...
          if %w(1 m).include?(gender)
            genders << 1
          elsif %w(0 f).include?(gender)
            genders << 0
          end
        end
    ...
    if @filters[:followup_status] && filtered_people.present?
      followup_statuses = @filters[:followup_status].split(',').collect { |x| x.delete(' ').underscore }
