...
class Element < ActiveRecord::Base
  ...
  TEXTFIELD_MATCH = [%w(Contains contains), ['Is Exactly', 'is_exactly'], ['Does Not Contain', 'does_not_contain'], ['No Response', 'is_blank'], ['Any Response', 'any']]
  CHOICEFILED_MATCH = [['Match Any', 'any'], ['Match All', 'all']]
  DATEFIELD_MATCH = [%w(Exact match), %w(Between between)]
  ...
  def search_people_answer_textfield(people, survey, answer, option = TEXTFIELD_MATCH.first[1].underscore, range = nil)
    ...
    if %w(contains is_exactly is_not_blank).include?(option)
  ...
  def required?(_answer_sheet = nil)
    super()
  end

  def position(survey = nil)
    if survey
      survey_elements.find_by(survey_id: survey.id).try(:position)
    else
      self[:position]
    end
  end
  ...
  def duplicate(survey, parent = nil)
    ...
    if parent.class.to_s == "ChoiceField"
