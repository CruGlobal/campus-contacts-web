class ApplicationController < ActionController::Base
  ...
  def unassigned_people(question_sheets)
    @unassigned_people ||= Person.who_answered(question_sheets).joins("LEFT OUTER JOIN contact_assignments ON contact_assignments.person_id = #{Person.table_name}.#{Person.primary_key}").where('contact_assignments.question_sheet_id' => question_sheets, 'contact_assignments.question_sheet_id' => nil)
  end
