...
class Api::ContactsController < ApiController
  ...
  def search_1
    ...
    unless @keywords.empty? || !params[:term].present?
      ...
      @people = Person.who_answered(@surveys).where('CONCAT(`people`.`first_name`," ", `people`.`last_name`) LIKE ? OR `people`.`last_name` LIKE ?', "%#{params[:term]}%", "%#{params[:term]}%")
  ...
  def search_2
    ...
    if params[:term].present?
      @people = Person.where('CONCAT(`people`.`first_name`," ", `people`.`last_name`) LIKE ? OR `people`.`last_name` LIKE ?', "%#{params[:term]}%", "%#{params[:term]}%")
      @people = paginate_filter_sort_people(@people, @organization)
  ...
  def index_2
    ...
    if params[:filters].present?
      ...
      if @filters.key? 'assigned_to'
        if @filters['assigned_to'] == 'none'
          ...
          if @filters['assigned_to'].is_a?(Array)
      ...
      if @filters.key? 'name'
        @people = @people.where('CONCAT(`people`.`first_name`," ", `people`.`last_name`) LIKE ? OR `people`.`last_name` LIKE ?', "%#{@filters['name']}%", "%#{@filters['name']}%")
      end
      if @filters.key? 'first_name'
        @people = @people.where('first_name like ?', '%' + @filters['first_name'] + '%')
      end
      if @filters.key? 'last_name'
        @people = @people.where('last_name like ?', '%' + @filters['last_name'] + '%')
      end
    ...
    if params[:order_by].present?
      ...
      ordering.each do |sort|
        ...
        dir = 'asc' if dir != 'asc' && dir != 'desc'

        case on
        when 'first_name'
          @people = @people.order("`#{Person.table_name}`.`first_name` #{dir}")
        when 'last_name'
          @people = @people.order("`#{Person.table_name}`.`last_name` #{dir}")
        when 'phone'
  ...
  def show_1
    ...
    @keys = @keywords.collect { |k| { name: k.keyword, keyword_id: k.id, questions: k.survey.questions.collect(&:id) } }
  ...
  def show_2
    ...
    @surveys = Survey.where(id: @all_survey_ids).collect { |s| { name: s.title, id: s.id, questions: s.questions.collect(&:id) } }
