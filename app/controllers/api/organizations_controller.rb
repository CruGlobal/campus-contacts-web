class Api::OrganizationsController < ApiController
  ...
  def show_1
    ...
    orgs = Organization.includes(:users, surveys: :questions).find(org_permissions.collect(&:organization_id))
    ...
    orgs.each do |org|
      ...
      # Leaders
      json[:leaders] = org.users.collect { |l| l.to_hash_mini_user(@organization.id) }
  ...
  def build_question_json(keyword_json, q, active)
    ...
    question_json[:required] = 0 if q.required.nil?
    question_json[:choices] = q.content.split(/[\r\n]/).compact.reject(&:empty?) unless q.content.nil? || q.kind != 'ChoiceField'
