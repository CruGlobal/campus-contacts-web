class Api::InteractionsController < ApiController
  ...
  before_action :valid_request_before, :authorized_leader?, :organization_allowed?, :get_organization, :get_api_json_header
  ...
  def create_1
    ...
    fail InteractionCreateParamsError if @json['interaction'].blank?
  ...
  def destroy_1
    fail InteractionDeleteParamsError unless params[:id].present? && (is_int?(params[:id]) || (params[:id].is_a? Array))
    ids = params[:id].split(',')
    ...
    interactions.each_with_index do |interaction, i|
      if permission[i] == 'user'
        fail InteractionPermissionsError unless interaction.created_by_id == current_person.id
      elsif permission[i] == 'admin'
        fail InteractionPermissionsError unless interaction.organization_id == @organization.id
      else
        fail InteractionPermissionsError
      end
    end
