class SurveysController < ApplicationController
  before_action :set_keyword_cookie, only: :start
  before_action :prepare_for_mobile, only: [:start, :stop, :index]
  ...
  def remove_logo
    ...
    if @survey.present?
      ...
      msg = I18n.t('surveys.form.remove_logo_success') if @survey.save
    end
  ...
  def copy_survey
    ...
    if @survey && @receiving_org
      if @receiving_org.surveys.where(title: @survey.title).present?
        ...
        @status = 'copied' if @survey.duplicate_to_org(@receiving_org)
      end
    end
  end
