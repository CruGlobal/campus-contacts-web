class Apis::V4::PeopleController < Apis::V4::BaseController
  ...
  def create
    clean_attr
    person = V4::Person.find_existing_person(V4::Person.new(params[:person]))
  ...
  def filtered_people(process = nil)
    unless @filtered_people
      ...
      if params[:filters]
        @filtered_people = PersonFilter.new(params[:filters], current_organization).filter(@filtered_people)
      end
