      ...
      wants.csv do
        ...
        CSV.generate(out) do |rows|
          ...
          @all_people.each do |person|
            if @roles[person.id]
              answers = [person.firstName, person.lastName, @roles[person.id].followup_status.to_s.titleize, person.assigned_tos(current_organization).collect{|a| Person.find(a.assigned_to_id).name}.join(','), person.gender.to_s.titleize, person.pretty_phone_number, person.email]
              dates = []
              @questions.each do |q|
