...
class ContactsController < ApplicationController
  ...
  rescue_from OrganizationalPermission::InvalidPersonAttributesError do |_exception|
  ...
  def clean_params(clean_all = false)
    ...
    if search? && clean_all
      ...
      session[:filters].each do |k, v|
        params[k] = v unless %w(controller action).include?(k)
  ...
  def export_csv
    if params[:only_ids].present? && params[:format].present?
      if params[:only_ids].present?
        ...
        send_data csv, filename: "#{@organization} - Contacts.csv", type: 'text/csv; charset=utf-8; header=present'
      else
  ...
  def update
    ...
    @valid_date = true
    if params[:answers]
      params[:answers].each do |_survey, answers|
        answers.each do |key, val|
          if val.present? && date_question = Element.where(id: key.to_i).first
            ...
            @person.save if %w(birth_date graduation_date).include?(date_question.attribute_name)
            unless @person.valid?
              @valid_date = false
              break
            end
        ...
        if @predefined_questions.collect(&:id).include?(question_id)
          if @question = @predefined_questions.find(question_id)
            ...
            if settings[:visible_predefined_questions].nil?
              settings[:visible_predefined_questions] = []
            end
  ...
  def unhide_question_column
    ...
    if @selected_question_id == 'visible_surveys_column'
      ...
      if @predefined_questions.collect(&:id).include?(@selected_question_id)
        ...
        settings = current_organization.settings
        settings[:visible_predefined_questions] = [] if settings[:visible_predefined_questions].nil?
        settings[:visible_predefined_questions] << @question.id
        current_organization.save(validate: false)
      else
  ...
  def prepare_pagination
    ...
    if params[:per_page].present?
      ...
      session[:per_page] ||= 25 unless session[:per_page].present?
    end
  end
  ...
  def sort_people(page = 1, fetch_all = false)
    ...
    if params[:search].present?
      ...
      if %w(dorm zip country room state address1 city).include?(sort_query.split(' ').first)
      ...
      if %w(last_name first_name gender).any? { |i| sort_query.include?(i) }
