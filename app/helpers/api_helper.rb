...
module ApiHelper
  ...
  def valid_request?(request) # , in_action = nil, prms = nil, accesstoken = nil
    ...
    fail ApiErrors::InvalidRequest if request.params[:action].nil? # && in_action.nil?
    ...
    if !param[:fields].nil?
      ...
      fail ApiErrors::InvalidFieldError if fields.empty?
  ...
  def valid_fields?(fields, action, version)
    ...
    fail "#{action} -- #{version}" if valid_fields.empty?
    valid_fields
  end

  def organization_allowed?
    if params[:org].present? || params[:org_id].present?
      fail ApiErrors::OrganizationNotIntegerError unless is_int?(params[:org_id]) || is_int?(params[:org])
      org_id = params[:org].present? ? params[:org].to_i : params[:org_id].to_i
      fail ApiErrors::OrgNotAllowedError unless valid_org_ids.include?(org_id)
    elsif params[:keyword].present?
      @valid_key_ids = valid_keywords.collect(&:id)
      fail ApiErrors::OrgNotAllowedError unless @valid_key_ids.include?(params[:keyword])
    elsif params[:keyword_id].present?
      fail ApiErrors::KeywordNotIntegerError unless is_int?(params[:keyword_id])
      @valid_key_names = valid_keywords.collect(&:keyword)
      fail ApiErrors::OrgNotAllowedError unless @valid_key_names.include?(params[:keyword])
    end
    true
  end
  ...
  def authorized_leader?
    fail ApiErrors::IncorrectPermissionsError unless can?(:lead, get_organization)
  end
  ...
  def get_people
    ...
    fail ApiErrors::NoDataReturned if people.empty?
    people
  end
  ...
  def get_organization
    if params[:org_id].present? || params[:org].present?
      fail OrganizationNotIntegerError unless is_int?(params[:org_id]) || is_int?(params[:org])
  ...
  def limit_and_offset_object(object)
    ...
    fail LimitRequiredWithStartError if params[:start].present? && !params[:limit].present?
  ...
  def paginate_filter_sort_people(people, organization)
    # settings for below
    allowed_sorting_fields = %w(time status)
    allowed_sorting_directions = %w(asc desc)
    allowed_filter_fields = %w(gender status)
    ...
    if params[:assigned_to].present? && (params[:assigned_to] == 'none' || params[:assigned_to].to_i > 0)
      if params[:assigned_to] == 'none'
        assigned_people_ids = people.joins(:assigned_tos).where('contact_assignments.organization_id' => organization.id).collect(&:id)
        people = people.where('people.id NOT IN (?)', assigned_people_ids) if assigned_people_ids.present?
      else
    ...
    if params[:filters].present? && params[:values].present?
      ...
      @filter_fields.each_with_index do |field, index|
        case field
        when 'gender'
          gender = (@filter_values[index].downcase == 'male') ? '1' : '0' if %w(male female).include?(@filter_values[index].downcase)
        ...
        when 'status'
          ...
          status = %w(uncontacted attempted_contact contacted) if status == 'not_finished'
  ...
  def render_json_error(exception = nil)
    ...
    finiteExceptions += oauthExceptions
