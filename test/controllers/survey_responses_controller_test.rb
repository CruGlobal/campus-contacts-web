...
class SurveyResponsesControllerTest < ActionController::TestCase
  ...
  context "Redirect to correct login option" do
    setup do
      ...
      @survey0 = FactoryGirl.create(:survey, login_option: 0)
      @survey1 = FactoryGirl.create(:survey, login_option: 1)
      @survey2 = FactoryGirl.create(:survey, login_option: 2)
      @survey3 = FactoryGirl.create(:survey, login_option: 3)
    end
  ...
  context "After logging in a person without orgs" do
    setup do
      ...
      @keyword = FactoryGirl.create(:approved_keyword, organization: @organization, survey: @survey)
    ...
    context "new with received_sms_id from mobile" do
      setup do
        @sms = FactoryGirl.create(:sms_session, sms_keyword: @keyword, person: @user.person)
  ...
  context "Answering a survey with notifications" do
    setup do
      ...
      @keyword = FactoryGirl.create(:approved_keyword, organization: @organization, survey: @survey)
      @notify_q = FactoryGirl.create(:choice_field, notify_via: "Both", trigger_words: "Jesus")
    ...
    should "respond success when answering survey" do
      @respondent = FactoryGirl.create(:user_no_org_with_facebook)
      FactoryGirl.create(:authentication, user: @respondent)
...
=begin
    should "merge people when sent params has an email that is existing in the db" do
      @respondent = FactoryGirl.create(:user_no_org_with_facebook)
      FactoryGirl.create(:authentication, user: @respondent)
      @email = FactoryGirl.create(:email_address, person: @respondent.person)
      @organization.add_contact(@respondent.person)
      @answer_sheet = FactoryGirl.create(:answer_sheet, survey: @survey, person: @respondent.person)
      @answer_to_choice = FactoryGirl.create(:answer_1, answer_sheet: @answer_sheet, question: @questions.first)
...
=end
    ...
    should "be able to update phone number to correct phone number when first input is wrong" do
      ...
      put :update, :survey_id => @survey.id, :id => @user.person.id, :person => {:phone_number => "123456789"}
    ...
    should "not create an answer sheet when the birth date is invalid date format" do
      assert_no_difference "Answer.count" do
        @birth_date_question = FactoryGirl.create(:text_field, object_name: 'person', attribute_name: 'birth_date')
