...
class LabelsControllerTest < ActionController::TestCase
  ...
  context 'After logging in a person with orgs' do
    ...
    should 'show all the system labels for cru org' do
      ...
      get :index
      system_labels = assigns(:system_labels).collect(&:i18n)
      organizational_labels = assigns(:organizational_labels).collect(&:i18n)
      assert_response :success, @response.body
      assert_equal %w(involved engaged_disciple leader), system_labels, @organization.default_labels
    end
