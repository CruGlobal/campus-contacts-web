...
class PeopleControllerTest < ActionController::TestCase
  ...
  context 'After logging in a person' do
    ...
    context 'bulk sending' do
      ...
      should 'update permissions' do
        ...
        permissions = permissions.collect(&:id).join(',')
  ...
  context 'When updating permissions' do
    ...
    should 'retain old permissions of different users even if users have initially have a different set of permissions' do
      ...
      org_permission.update_attributes(archive_date: nil,
                                       added_by_id: @user.person.id,
                                       deleted_at: nil,
                                       permission_id: 3)

      old_person_2_permissions = @person2.organizational_permissions.where(organization_id: @org.id).collect(&:permission_id)
      old_person_3_permissions = @person3.organizational_permissions.where(organization_id: @org.id).collect(&:permission_id)
      ...
      new_person_2_permissions = @person2.organizational_permissions.where(organization_id: @org.id).collect(&:permission_id)
      new_person_3_permissions = @person3.organizational_permissions.where(organization_id: @org.id).collect(&:permission_id)
  ...
  context "Clicking on the label links at '/people'" do
    ...
    should 'return admins when Admin link is clicked' do
      ...
      assert_equal [@admin1.person.id, @user.person.id], assigns(:all_people).collect(&:id)
    end

    should 'return leaders when MissionHub Users link is clicked' do
      xhr :get, :index, permission: Permission::USER_ID
      assert_equal [@leader1.person.id], assigns(:all_people).collect(&:id)
    end

    should 'return contacts when Contact link is clicked' do
      xhr :get, :index, permission: Permission::NO_PERMISSIONS_ID
      assert_equal [@contact1.person.id, @contact2.person.id, @contact3.person.id].sort { |x, y| x <=> y }, assigns(:all_people).collect(&:id)
    end
    ...
    should 'return archiveds when Archived link is clicked' do
      ...
      assert_equal assigns(:all_people).collect(&:id), [@contact1.person.id, @contact2.person.id, @contact3.person.id].sort { |x, y| x <=> y }
    end
    ...
    should "only return unarchived people when 'Include Archived' checkbox isn't checked'" do
      ...
      xhr :get, :index
      assert_equal [@admin1.person.id, @user.person.id, @contact3.person.id], assigns(:all_people).collect(&:id)
    end
    ...
    should "return all people (even unarchived ones) when 'Include Archived' checkbox is checked'" do
      ...
      assert_equal [@admin1.person.id, @leader1.person.id, @contact1.person.id, @contact2.person.id, @user.person.id, @contact3.person.id], assigns(:all_people).collect(&:id)
    end
