...
class LeadersControllerTest < ActionController::TestCase
  context 'Leader Link is clicked' do
    setup do
      @leader_user = FactoryGirl.create(:user_with_auxs, remember_token: '123456', remember_token_expires_at: Date.today + 10.day)
    end
    ...
    should "redirect to user root if there's no token and user_id when visiting the merge_leader_link" do
      ...
      sign_in @user
      @leader_user.update_attribute(:remember_token_expires_at, Date.today - 1.day)
      get :merge_leader_accounts, user_id: '', token: @leader_user.remember_token
      assert_response :redirect
    end
  ...
  context 'After logging in a person with orgs' do
    ...
    should 'update a person and add them as a leader' do
      person = FactoryGirl.create(:person)
      person.email = 'bad_old@email.com'
      person.user = nil
      person.save(validate: false)
      assert_difference('OrganizationalPermission.count') do
        assert_difference('User.count') do
    ...
    should 'should remove a leader' do
      ...
      FactoryGirl.create(:email_address, email: 'user3@email.com', person: user3.person)
      user2.person.reload
      user3.person.reload
    ...
    should 'not be able to sign in leader' do
      get :leader_sign_in, user_id: '', token: ''
  ...
  context 'Seaching for Persons to be assigned a leader permission' do
    ...
    should 'successfully find a searched Person if Person has valid email' do
      FactoryGirl.create(:person, first_name: 'NeilMarion', email: 'super_duper_unique_email@mail.com')
