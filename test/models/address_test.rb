...
class AddressTest < ActiveSupport::TestCase
  ...
  context "htmlify address" do
    ...
    should "output correct text if address1 and 2 are not present" do
      a = Address.create(zip: "6000", country: "Philippines", address_type: "current", person_id: @person.id)
  ...
  should "create map link" do
    @person = FactoryGirl.create(:person)
    FactoryGirl.create(:email_address, person: @person, email: "person1@email.com")
    a = Address.new(:address1 => "Cebu City", :country => "Philippines", :zip => "6000", :address_type => "current", :person_id => @person.id)
    a.save

    assert_equal "http://maps.google.com/maps?f=q&source=s_q&hl=en&q=Cebu City+6000+Philippines", Address.last.try(:map_link)
  end
