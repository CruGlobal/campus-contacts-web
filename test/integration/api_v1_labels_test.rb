...
class ApiV1LabelsTest < ActionDispatch::IntegrationTest
  context "the api" do
    ...
    should "return a JSON error if the identity of the access token is not an admin" do
      path = "/api/labels/#{@user.person.id}"
      @user3.person.organizational_labels.first.update_attributes(label_id: Label::ALUMNI_ID)
    ...
    should "successfully update a person from contact to leader status" do
      path = "/api/labels/#{@user.person.id}"
      @user.person.organizational_labels.first.update_attributes(label_id: Label::ALUMNI_ID)
    ...
    should "successfully update a person from leader to contact status" do
      ...
      Factory(:organizational_label, person: @user.person, label: Label.alumni, organization: @user3.person.primary_organization, :added_by_id => user2.person.id)
