  ...
  context "the api" do
    ...
    should "be able to request person information" do
     ...
     should "be able to request person information with fields" do
       path = "/api/v1/people/#{@user3.person.id}"
       get path, {'access_token' => @access_token3.code, 'fields' => "first_name,last_name,name,id,birthday,fb_id,picture,gender,education,interests,id,locale,location,assignment,request_org_id,organization_membership,organizational_permissions,status"}, {:accept => 'application/vnd.missionhub-v1+json'}
       assert_response :success, @response.body
       @json = ActiveSupport::JSON.decode(@response.body)
