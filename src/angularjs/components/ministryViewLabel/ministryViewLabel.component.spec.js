...
describe('minstryViewLabel component', function() {
  ...
  describe('editLabel', function() {
    it('should open the edit label modal', function() {
      ...
      expect(
        this.$uibModal.open.calls.argsFor(0)[0].resolve.organizationId(),
      ).toEqual(this.organization.id);
      expect(this.$uibModal.open.calls.argsFor(0)[0].resolve.label()).toEqual(
        this.label,
      );
    });
  });
    ...
    it(
      ...
      asynchronous(function() {
        ...
        return $ctrl.deleteLabel().then(function() {
          expect(_this.labelsService.deleteLabel).toHaveBeenCalledWith(
            _this.label,
          );
        });
      }),
