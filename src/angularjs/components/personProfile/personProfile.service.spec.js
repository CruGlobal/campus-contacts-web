...
describe('personProfileService', function() {
  ...
  describe('saveRelationship', function() {
    ...
    it('should add created relationships on unpersisted people', function() {
      ...
      expect(_.map(this.person.relationships, 'key')).toEqual([
        'value',
        'value2',
      ]);
    });
  ...
  describe('deleteRelationship', function() {
    it('should call deleteRelationships', function() {
      ...
      expect(personProfileService.deleteRelationships).toHaveBeenCalledWith(
        this.person,
        [this.relationship],
        'relationships',
      );
    ...
    it(
      ...
      asynchronous(function() {
        ...
        return personProfileService
          .deleteRelationships(this.person, this.relationships, 'relationships')
          .then(function() {
  ...
  describe('addAssignments', function() {
    ...
    it('should save assignments locally when adding assignments to an unpersisted person', function() {
      ...
      var assignedToIds = _
        .chain(this.person.reverse_contact_assignments)
  ...
  describe('removeAssignments', function() {
    ...
    it('should make a network request for each removed assignment', function() {
      ...
      var errorMessageConfig = jasmine.objectContaining({
        errorMessage: jasmine.any(String),
      });
    ...
    it(
      ...
      asynchronous(function() {
        ...
        return personProfileService
          ...
          .then(function() {
            expect(
              _.map(_this.person.reverse_contact_assignments, 'id'),
            ).toEqual([2, 4]);
          });
      }),
    );
  ...
  describe('formatAddress', function() {
    ...
    it('should generate the region line', function() {
      var tests = [
        {
          ...
          lines: ['Orlando, FL 32832'],
        },
        { address: { city: 'Orlando', state: 'FL' }, lines: ['Orlando, FL'] },
        {
          address: { city: 'Orlando', zip: '32832' },
          lines: ['Orlando 32832'],
        },
      ...
      tests.forEach(function(test) {
        expect(personProfileService.formatAddress(test.address)).toEqual(
          test.lines,
        );
      });
    });
    ...
    it('should hide non-US country lines', function() {
      expect(personProfileService.formatAddress({ country: 'CA' })).toEqual([
        'CA',
      ]);
