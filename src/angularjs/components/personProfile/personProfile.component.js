...
function personProfileController(
  ...
  function activate() {
    ...
    vm.disableNameFields =
      vm.personTab.person.user && loggedInPerson.person !== vm.personTab.person;
    ...
    var organization = JsonApiDataStore.store.find(
      'organization',
    ...
    if (loggedInPerson.person === vm.personTab.person) {
      ...
      if (
        vm.personTab.orgPermission.permission_id === permissionService.adminId
      ) {
        vm.permissionChangeDisabled = true;
      } else {
  ...
  function isContact() {
    return personService.isContact(
      vm.personTab.person,
      vm.personTab.organizationId,
    );
  }
  ...
  function isEmailAddressValid(emailAddress) {
    ...
    return (
      control.$valid ||
      (control.$viewValue === '' && vm.isPendingEmailAddress(emailAddress))
    );
  }
  ...
  function isPhoneNumberValid(phoneNumber) {
    ...
    return (
      control.$valid ||
      (control.$viewValue === '' && vm.isPendingPhoneNumber(phoneNumber))
    );
  }
