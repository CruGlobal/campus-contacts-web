...
function organizationOverviewPeopleController(
    ...
    $uibModal
      ...
      .result.then(function() {
        ...
        loadPersonPage().then(function() {
          ...
          vm.selectAllValue =
            originalSelectAllValue && getSelectedPeople().length > 0;
        });
      });
  }
  ...
  function removePeople(message, performRemoval) {
    ...
    if (_.isEqual(selection.selectedPeople, [loggedInPerson.person.id])) {
      ...
      if (
        _.indexOf(selection.unselectedPeople, loggedInPerson.person.id) === -1
      ) {
        selection.unselectedPeople = _.union(selection.unselectedPeople, [
