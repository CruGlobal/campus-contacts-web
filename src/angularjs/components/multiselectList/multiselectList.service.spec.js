  ...
  describe('state checks', function() {
    ...
    describe('isSelected', function() {
      it('should return true for selected states', function() {
        expect(multiselectListService.isSelected(this.selected, this.id)).toBe(
          true,
        );
      });
      ...
      it('should return false for other states', function() {
        ...
        expect(
          multiselectListService.isSelected(this.indeterminate, this.id),
        ).toBe(false);
        expect(multiselectListService.isSelected(this.missing, this.id)).toBe(
          false,
        );
      });
    });
    ...
    describe('isUnselected', function() {
      it('should return true for unselected and missing states', function() {
        expect(
          multiselectListService.isUnselected(this.unselected, this.id),
        ).toBe(true);
        expect(multiselectListService.isUnselected(this.missing, this.id)).toBe(
          true,
        );
      });
      ...
      it('should return false for other states', function() {
        ...
        expect(
          multiselectListService.isUnselected(this.indeterminate, this.id),
        ).toBe(false);
      });
    });
    ...
    describe('isIndeterminate', function() {
      it('should return true for indeterminate states', function() {
        expect(
          multiselectListService.isIndeterminate(this.indeterminate, this.id),
        ).toBe(true);
      });

      it('should return false for other states', function() {
        expect(
          multiselectListService.isIndeterminate(this.selected, this.id),
        ).toBe(false);
        expect(
          multiselectListService.isIndeterminate(this.unselected, this.id),
        ).toBe(false);
        expect(
          multiselectListService.isIndeterminate(this.missing, this.id),
        ).toBe(false);
      });
    });
  });
