...
resource 'Apis::V4::Sessions' do
  ...
  post '/apis/v4/sessions' do
    ...
    response_field :token, 'MH JSON Web Token that will contain a user_id and an expires at'

    let(:provider) { 'facebook' }
    let(:code) { 'asdfasdfasdf' }
    ...
    example 'logging in' do
      ...
      person.create_user!
      user = person.user
      stub_request(:get, "https://graph.facebook.com/v2.3/oauth/access_token?client_id=#{ENV['FB_APP_ID']}"\
                           "&client_secret=#{ENV['FB_SECRET']}&code=asdfasdfasdf&redirect_uri=#{ENV['FB_REDIRECT_URI']}")
      ...
      token = JWT.decode(JSON.parse(response_body)['token'], ENV['JSON_WEB_TOKEN_SECRET'], true, algorithm: 'HS256')[0]
