  ...
  task "recruit" => :environment do
    ...
    people.each do |person_id, answers|
      if answers['contact_me']
        answer_sheet = AnswerSheet.where(person_id: person_id, survey_id: survey_id).first_or_create

        if answers['grad']
          grad_answer = Answer.where(answer_sheet_id: answer_sheet.id, question_id: grad_q.id).first_or_create
        ...
        conference_answer = Answer.where(answer_sheet_id: answer_sheet.id, question_id: conference_q.id).first_or_create
        conference_answer.update_attribute(:value, answers['conference_name'])

        contact_answer = Answer.where(answer_sheet_id: answer_sheet.id, question_id: contact_q.id).first_or_create
    ...
    question_usage_ids.each do |question_usage_id|
      ...
      ActiveRecord::Base.connection.select_all(sql).each do |row|
        ...
        if stripped_num.length == 10
          ...
          person.phone_numbers.where(number: stripped_num).first || person.phone_numbers.create(number: stripped_num, location: 'mobile')
        end
      end
    end
  end
